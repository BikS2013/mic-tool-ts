...
### FR-31.1 — Clearable grouped transcript timeline
The UI transcript timeline MUST expose a clear action that removes only the renderer's visible transcript history and live partial text. Clearing the timeline MUST NOT stop an active listening session, modify protocol operators, or persist transcript state. Dictated text from a single submitted or closed turn MUST be collected into one raw dictated-text bubble, and refined or translated results for that same turn MUST appear as follow-up processed-output bubbles in chronological order inside the same grouped timeline entry.
...
If a push-to-talk session remains continuously in warm/ready state for five minutes, Electron main MUST recycle it by stopping the existing hotkey-owned session through the normal shutdown path and then starting a fresh warm session when push-to-talk is still enabled. The recycle timer MUST be cancelled while the hotkey is actively recording, when the session stops, when push-to-talk is disabled, or when a manual session owns the runner.
...
### FR-35 — Hotkey transcription overlay
When UI push-to-talk recording starts, the Electron UI MUST show an independent bottom-center overlay window that is separate from the main UI `BrowserWindow`. The overlay MUST be owned by Electron main, load only local packaged renderer content, use the same sandboxed/context-isolated preload bridge security posture as the main renderer, and remain useful when the main UI is hidden, covered, or unfocused.

The overlay MUST be driven only by typed UI events from the existing session pipeline. It MUST show on hotkey-owned `capture.state: recording`, replace live partial text in place on `transcript.partial`, briefly show committed text on `transcript.final`, optionally show processed output or warnings/errors briefly, and hide after release or stop. It MUST NOT show for manual Start/Stop sessions. It MUST clear displayed transcript text on hide and MUST NOT persist transcript text, processed output, protocol events, provider endpoints, API keys, or secret values.

The overlay MUST avoid stealing focus from the foreground application. It SHOULD use a display-only frameless Electron window with skip-taskbar, non-focusable, always-on-top behavior and bottom-center placement based on the work area of the display nearest the cursor at recording start. Long transcript text MUST wrap/clamp inside a stable compact layout without resizing the overlay.

The overlay MUST include compact bottom indicators for the current protocol operator state: refinement, translation, clipboard copy, and focused-input delivery. The indicators MUST update from typed protocol state, including spoken `command <operator>` changes and runtime hotkey changes, and MUST NOT persist transcript text, processed output, or protocol event payloads.

While a UI push-to-talk hotkey is held, the user MUST be able to toggle protocol operators with secondary keys: `R` toggles `refine`, `T` toggles `translate`, `C` toggles `clipboard`, and `I` toggles `input`. These toggles MUST affect the active hotkey-owned protocol controller before the pending section is submitted on release, and MUST emit normal `state.changed` protocol events so the main UI and overlay stay synchronized.
